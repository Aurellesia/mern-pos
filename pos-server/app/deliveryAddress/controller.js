...
const update = async (req, res, next) => {
  try {
    ...
    console.log(`Subject address : ${JSON.stringify(subjectAddress)}`);

    let policy = policyFor(req.user);
    if (!policy.can("update", subjectAddress)) {
      return res.json({
        error: 1,
        message: `You're not allowed to modify this recource`,
      });
    }
...
const destroy = async (req, res, next) => {
  ...
  try {
    ...
    if (!policy.can("delete", subjectAddress)) {
      return res.json({
        error: 1,
        message: `You're not allowed to this this recource`,
      });
    }
